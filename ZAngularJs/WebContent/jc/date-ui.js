...
/***/ function(module, exports, __webpack_require__) {
	...
	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { defaultT: obj }; }

	// sets up jQuery with the datepicker plugin

	exports.defaultT = _angular2.defaultT.module('ui.date', []).constant('uiDateConfig', {}).constant('uiDateFormatConfig', '').factory('uiDateConverter', ['uiDateFormatConfig', function (uiDateFormatConfig) {
	  return {
	  ...
	  function dateToString(uiDateFormat, value) {
	    ...
	    if (value) {
	      if (dateFormat) {
	        try {
	          return _jquery2.defaultT.datepicker.formatDate(dateFormat, value);
	  ...
	  function stringToDate(dateFormat, valueToParse) {
	    ...
	    if (_angular2.defaultT.isDate(valueToParse) && !isNaN(valueToParse)) {
	      return valueToParse;
	    }

	    if (_angular2.defaultT.isString(valueToParse)) {
	      if (dateFormat) {
	        return _jquery2.defaultT.datepicker.parseDate(dateFormat, valueToParse);
	      }
	    ...
	    if (_angular2.defaultT.isNumber(valueToParse)) {
	  ...
	  return {
	    ...
	    link: function link(scope, element, attrs, controller) {
	      ...
	      var $element = (0, _jquery2.defaultT)(element);

	      var getOptions = function getOptions() {
	        return _angular2.defaultT.extend({}, uiDateConfig, scope.$eval(attrs.uiDate));
	      };
	      var initDateWidget = function initDateWidget() {
	        ...
	        function setVal(forcedUpdate) {
	          ...
	          var isDate = _angular2.defaultT.isDate(controller.$modelValue);
	          var preserve = {};
	          ...
	          if (isDate) {
	            _angular2.defaultT.forEach(keys, function (key) {
	        ...
	        if (controller) {
	          ...
	          var _onSelect = opts.onSelect || _angular2.defaultT.noop;
	          ...
	          var _beforeShow = opts.beforeShow || _angular2.defaultT.noop;
	          opts.beforeShow = function (input, picker) {
	            showing = true;
	            _beforeShow(input, picker, $element);
	          };

	          var _onClose = opts.onClose || _angular2.defaultT.noop;
	          ...
	          controller.$validators.uiDateValidator = function uiDateValidator(modelValue, viewValue) {
	            return viewValue === null || viewValue === '' || _angular2.defaultT.isDate(uiDateConverter.stringToDate(attrs.uiDateFormat, viewValue));
	          };
	          ...
	          controller.$render = function () {
	            ...
	            if (_angular2.defaultT.isDate(controller.$modelValue) === false && _angular2.defaultT.isString(controller.$modelValue)) {
